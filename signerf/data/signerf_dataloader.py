...
class EvalCameraDataloader(DataLoader):
    ...
    def __init__(
        self,
        camera_dataset: CameraArcDataset,
        device: Union[torch.device, str] = "cpu",
        **kwargs,
    ):
        self.camera_dataset = camera_dataset
        self.cameras = camera_dataset.cameras.to(device)
        self.device = device
        self.kwargs = kwargs
        super().__init__(dataset=camera_dataset)
    ...
    def get_camera(self, camera_idx: int = 0) -> Cameras:
    ...
    def get_data_from_idx(self, camera_idx: int) -> RayBundle:
        """Returns the data for a specific image index.

        Args:
            image_idx: Camera image index
        """
        ray_bundle = self.cameras.generate_rays(camera_indices=camera_idx, keep_shape=True)

        return ray_bundle


class FixedIndicesEvalCameraDataloader(EvalCameraDataloader):
    ...
    def __init__(
        self,
        camera_dataset: CameraArcDataset,
        camera_indices: Optional[Tuple[int]] = None,
        device: Union[torch.device, str] = "cpu",
        **kwargs,
    ):
        super().__init__(camera_dataset, device, **kwargs)
        if camera_indices is None:
            self.camera_indices = list(range(len(camera_dataset)))
        else:
    ...
    def __next__(self):
        if self.count < len(self.camera_indices):
            camera_idx = self.camera_indices[self.count]
            ray_bundle = self.get_data_from_idx(camera_idx)
            self.count += 1
            return ray_bundle, camera_idx
        raise StopIteration

class SelectiveCacheDataloader(DataLoader):
    ...
    def __init__(
        ...
        indices_to_sample_from: Optional[Sequence[int]] = None,
        device: Union[torch.device, str] = "cpu",
        ...
        **kwargs,
        ...
        # Sequence of indices to sample from. If None, then sample from all images.
        self.indices_to_sample_from = range(len(self.dataset)) if indices_to_sample_from is None else indices_to_sample_from
    ...
    def _get_batch_list(self):
        ...
        indices = random.sample(self.indices_to_sample_from, k=len(self.indices_to_sample_from))
        batch_list = []
        results = []
    ...
    def _get_collated_batch(self):
        ...
        # Convert collated_batch "image" array of tensors to a single tensor
        # collated_batch["image"] = torch.stack(collated_batch["image"], dim=0) # FIXME: this is a hack and should only be done if it is a list of tensors
